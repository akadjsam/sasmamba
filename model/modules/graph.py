...
class MultiOrderGCN(nn.Module):
    ...
    def _init_spatial_adj(self):
        adj = torch.eye(self.num_nodes)  # 加上自环
        connections = self.connections if self.connections is not None else CONNECTIONS
        for i in range(self.num_nodes):
            connected_nodes = connections[i]
            for j in connected_nodes:
    ...
    def _init_temporal_adj(self, connection_length):
        ...
        for i in range(self.num_nodes):
            for j in range(1, connection_length + 1):
    ...
    def normalize_digraph(adj):
        ...
        deg_inv_sqrt[node_degrees == 0] = 0
        norm_deg = torch.eye(adj.size(1), device=adj.device).view(1, adj.size(1), adj.size(1)) * deg_inv_sqrt.unsqueeze(-1)
        return deg_inv_sqrt.unsqueeze(1) * adj * deg_inv_sqrt.unsqueeze(-1)

    def change_adj_device(self, adj):
    ...
    def forward(self, x):
        ...
        if self.mode == 'temporal':
            ...
            if self.use_temporal_similarity:
                ...
                adj = self.adj
                adj = self.change_adj_device(adj)
                adj = adj.repeat(b * j, 1, 1)
        else:
        ...
        # 一阶、二阶、三阶邻接矩阵
        adj1 = adj
        adj2 = torch.bmm(adj1, adj1)
        adj3 = torch.bmm(adj2, adj1)

        adj1 = self.normalize_digraph(adj1)
        adj2 = self.normalize_digraph(adj2)
        adj3 = self.normalize_digraph(adj3)

        # 聚合
        x_v = self.V(x)  # (B*T, J, C_out)
        agg1 = torch.bmm(adj1, x_v)
        agg2 = torch.bmm(adj2, x_v)
        agg3 = torch.bmm(adj3, x_v)

        aggregate = self.alpha[0] * agg1 + self.alpha[1] * agg2 + self.alpha[2] * agg3
        ...
        if self.mode == 'spatial':
            ...
            x = x.reshape(-1, j, t, self.dim_out).transpose(1, 2)
        return x
