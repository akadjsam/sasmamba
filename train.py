...
from utils.learning import AverageMeter, decay_lr_exponentially,load_model_ssdmamba
...
def train(args, opts):
    ...
    model = load_model_ssdmamba(args)
    ...
    #         warmup_lr = base_lr * (epoch + 1) / warmup_epochs  # 线性增长
    #         for param_group in optimizer.param_groups:
    #             param_group['lr'] = warmup_lr
    #         print(f"Epoch {epoch}: warmup lr = {warmup_lr:.6f}")
    #     else:
    #         # 正常训练期，保持 base_lr（或你可以开始使用 decay 策略）
    #         for param_group in optimizer.param_groups:
    #             param_group['lr'] = base_lr
    #         print(f"Epoch {epoch}: lr = {base_lr:.6f}")
    #
    #     # 你的训练过程
